...
## 18. Coal Switching Price ----

vec_plot_line = excel_file_sn[c(9)]

report_num_transformed = sub("([A-Za-z]+)([0-9]{4})$", "\\1Q\\2", report_num)

color_mapping = c(`CSP Avg. Efficiency` = "#00544F", 
                  `PSV + Logistics` = "#FF9933",
                  `CSP Range` = "#E8E8E8",
                  `CSP min` = "#E8E8E8")

color_mapping[report_num] = "#FF9933"
color_mapping[paste('CSP Range',report_num_transformed)] = "#000000"
color_mapping[paste('CSP Range',report_num_transformed,'min')] = "#000000"

levels_order = c("CSP Range", "CSP Avg. Efficiency", "PSV + Logistics",paste('CSP Range',report_num_transformed),report_num)

linetype_mapping = c(`CSP Avg. Efficiency` = "solid", 
                  `PSV + Logistics` = "solid",
                  `CSP Range` = "solid",
                  `CSP min` = "solid")

linetype_mapping[report_num] = "dashed"
linetype_mapping[paste('CSP Range',report_num_transformed)] = "dashed"
linetype_mapping[paste('CSP Range',report_num_transformed,'min')] = "dashed"
...
for (i in 1:length(vec_plot_line)) { #i = 1
    ...
    reshaped_data <- dcast(dt_line_lg, ... ~ `Coal.Switching.Price,.Reference.Scenario.(€/MWh)`, value.var = "valori")
    
    
    plot_line =
        reshaped_data %>%
        ggplot(aes(x = anni)) +
        geom_ribbon(aes(ymin = `CSP min`, ymax = `CSP Range`, fill = "CSP Range")) + 
        geom_line(aes(y = `CSP Avg. Efficiency`,color = 'CSP Avg. Efficiency'), linewidth = 1.1,linetype = 'solid') +
        geom_line(aes(y = `PSV + Logistics`, color = 'PSV + Logistics'), linewidth = 1.1,linetype = 'solid') +
        geom_line(aes(y = .data[[paste('CSP Range',report_num_transformed)]], color = paste('CSP Range',report_num_transformed)), linewidth = 1.1, linetype = "dashed")+
        geom_line(aes(y = get(paste('CSP Range',report_num_transformed,'min')), color = paste('CSP Range',report_num_transformed,'min')), linewidth = 1.1,  linetype = "dashed") +
        geom_line(aes(y = get(report_num), color = report_num), linewidth = 1.1,  linetype = "dashed") +
        scale_colour_manual(values=color_mapping, breaks = levels_order)+
        scale_fill_manual("",values=c("#E8E8E8")) + 
        #scale_linetype_manual(values = c("PSV + Logistics" = "dashed")) +
        theme_light()
