...
will be removed from the market in 2024, which will limit supply
and push prices upward. Starting from 2025, the inclusion
of the maritime transport sector will increase demand. However,
...
::: callout-right
A notable supply-demand gap widening should start
...
::: callout-main
**IMPORTANT GRID REINFORCEMENTS TO BE REALIZED ALREADY IN THE MID 2020s. MAJOR IMPROVEMENTS ARE EXPECTED TO BE COMPLETED IN THE 2030s – TYRRHENIAN LINK AND ADRIATIC LINK.**
:::

```{r, echo=FALSE, out.height="50%", fig.cap="A nice image."}
knitr::include_graphics(file.path("figs/60. Potenziamento Rete.png"))
```
...
::: callout-right
Grid reinforcements are expected to improve grid flow management and reduce congestion between mainland zones. By 2030, two significant HVDC infrastructures are set to be operational: the Tyrrhenian Link, crucial for replacing Sardinian coal-fired units, and the Adriac Link, to ease bottlenecks between southern and northern regions. There may be delays compared to the proposed timeline by Terna and ARERA. In the Low scenario completion is assumed by 2028, while in the High scenario, is expected by 2035.
:::
:::
...
::: callout-right
In the Business as Usual (BAU) scenario, mainland inter-zonal constraints are expected to progressively decrease from 2035 onward. In the Low scenario, Terna's proposed Hypergrid project is assumed to be implemented, reaching a 30GW exchange capacity between market zones. In the High scenario, long-term network development is significantly limited.
:::
:::
...
::: callout-right
BAU grid development is based on Terna's 2023 Development Plan. Assumptions in alternative scenarios are align with other boundary conditions.
:::
:::
